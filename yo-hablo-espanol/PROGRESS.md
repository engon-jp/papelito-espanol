...
- **出題**：各レベル独立の25問ラウンド。全48問からランダム抽出。`SAVE_KEY=es-trainer-v2`。
...
- **サマリ＋復習（2026-07-02追加）**：25問終了で `renderSummary()`（正解数・%・段階別メッセージ・誤答一覧）。「間違えた問題を復習」で誤答だけの復習ラウンド（HUD表示は「復習」、復習後もサマリ→再復習可）、「新しいラウンドへ」で新規25問を抽選。
