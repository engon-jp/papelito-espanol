...
- 生成しうる文の総数：**Nivel 1/2（単文）= 16主語 × 6補語 × 3パターン(肯定/否定/疑問) = 288通り**、**Nivel 3/4（一問一答）= 12主語 × 6補語 × 2(はい/いいえ) = 144通り**。無限ではなく上限のある組み合わせ空間（合計約432通り）。
- **2026-07-02：Nivel 1/2 の出題主語に 彼ら(Ellos)/彼女ら(Ellas) を追加**（14→16主語）。従来の除外は意図したものではなく作成時の抜けだったため（遠藤さん確認済み）。QA（Nivel 3/4）には従来から含まれている。
